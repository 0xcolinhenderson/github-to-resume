export async function POST(request) {
  try {
    ...
    if (response.ok) {
      ...
      return new Response(JSON.stringify({ message: "Repository is valid", success: true }), {
        status: 200,
      });
    } else {
    ...
    return new Response(JSON.stringify({ message: "Failed to check repository" }), { status: 500 });
  }
}
